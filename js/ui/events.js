...
export function renderEventList(items, emptyText) {
  ...
  return items.map(item => `
    <article class="card event-card ${item.severity || "advisory"}">
      ...
      <p class="event-meta">Confidence: ${escapeHtml(item.confidence || "Unknown")} · Status: ${escapeHtml(item.lifecycle || "active")} · Updated: ${relativeTime(item.lastUpdated)}</p>
      ...
      <div class="source-list">
        ${(item.sources || [{ name: item.source, link: item.link }]).slice(0, 5).map(source =>
          source.link
            ? `<a class="source-pill" href="${escapeAttr(source.link)}" target="_blank" rel="noopener">${escapeHtml(source.name || "Source")}</a>`
            : `<span class="source-pill">${escapeHtml(source.name || "Source")}</span>`
        ).join("")}
      </div>
    </article>
  `).join("");
