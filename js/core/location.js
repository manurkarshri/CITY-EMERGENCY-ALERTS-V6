...
export function setupLocationSelector() {
  ...
  region.innerHTML = Object.entries(state.regions || {})
    .map(([key, value]) => `<option value="${key}">${escapeHtml(value.label || key)}</option>`)
    .join("");
  region.value = state.selected.region;

  taluka.innerHTML = Object.entries(state.talukas || {})
    .map(([key, value]) => `<option value="${key}">${escapeHtml(value.label || key)}</option>`)
    .join("");
  ...
  function refreshLocalities() {
    ...
    locality.innerHTML = `<option value="">All localities</option>` +
